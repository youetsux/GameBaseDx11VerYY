...
//#endif
	...
	while (msg.message != WM_QUIT)
	{
		...
		else
		{
			...
			if ((nowTime - lastUpdateTime) * fpsLimit > 1000.0f)
			{
				...
				// ===== 影描画パス =====
				// 光源の視点からシーンを描画してシャドウマップを作る
				if (ShadowMap::IsEnabled())
				{
					ShadowMap::BeginShadowPass();	// 描画先をシャドウマップに切り替え
				...
				// シャドウマップを通常描画用シェーダーの t1 スロットにセット
				ShadowMap::BindToShader();
				...
				// シャドウマップの割り当てを解除（次フレームの書き込みのため）
				ShadowMap::UnbindFromShader();
	...
	ShadowMap::Release();	// シャドウマップ用リソースの解放
	Direct3D::Release();
