...
namespace Model
{
	...
	{
		if (handle < 0 || handle >= (int)_datas.size() || _datas[handle] == nullptr)
			return AABB();
		if (_datas[handle]->pFbx == nullptr)
			return AABB();
		return _datas[handle]->pFbx->GetAABB();
	}

	// 内部データへのポインタを取得
	const ModelData* GetData(int handle)
	{
		if (handle < 0 || handle >= (int)_datas.size())
			return nullptr;
